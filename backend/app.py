            ...
            try:
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        # 可能的照片命名格式
                        possible_names = [
                            f"{class_name}{seat_number}.jpg",
                            f"{class_name}{seat_number:02d}.jpg",  # 兩位數格式
                            f"{class_name}{seat_number}.png",
                            f"{class_name}{seat_number:02d}.png",  # 兩位數格式
                            f"{seat_number}.jpg",  # 只有座號
                            f"{seat_number:02d}.jpg",  # 兩位數座號
                            f"{seat_number}.png",
                            f"{seat_number:02d}.png"
                        ]

                        for name_try in possible_names:
                            photo_path = os.path.join(app.config['PHOTOS_FOLDER'], name_try)
                            if os.path.exists(photo_path):
                                photo_filename = name_try
                                photo_exists = True
                                break
...
def get_photo(class_name, seat_number):
    # 嘗試多種照片命名格式
    possible_names = [
        f"{class_name}{seat_number}",
        f"{class_name}{seat_number:02d}",  # 兩位數格式
        f"{seat_number}",  # 只有座號
        f"{seat_number:02d}"  # 兩位數座號
    ]

    # 嘗試不同的圖片格式
    for name in possible_names:
        for ext in ['.jpg', '.jpeg', '.png', '.gif']:
            photo_path = os.path.join(app.config['PHOTOS_FOLDER'], name + ext)
            if os.path.exists(photo_path):
